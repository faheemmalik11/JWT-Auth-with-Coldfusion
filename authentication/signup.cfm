    ...
    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
        <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form class="space-y-4 md:space-y-6" id="signUpForm" >
                    <div>
                    ...
                    <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                        Already have an account? <a href="/ecommerce-store/authentication/login.cfm" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Login here</a>
                    </p>
                </form>
...
<script>
    $(document).ready(function(){
        $('#signUpForm').on('submit', function(e){
            ...
            $.ajax({
                type: 'POST',
                url: '/ecommerce-store/authentication/auth.cfc?method=signup',
                data: {
                    name: name,
                    email: email,
                    password: password
                ...
                success: function(signUpResponse) {
                    var parsed = $.parseJSON(signUpResponse);
                        if(parsed.code == 200){
                            $.cookie("auth_user", JSON.stringify(parsed.data),{ expires: 7, path: '/' });
                                $.cookie("auth_token", JSON.stringify(parsed.token),{ expires: 7, path: '/' });
                            window.location.href = '/ecommerce-store/dashboard/index.cfm';
                        }
                    },
                error: function(signUpError) {
                        console.error("Error querying database: ", signUpError);
                }
