            ...
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form class="space-y-4 md:space-y-6" id="logInForm" >
                    ...
                    <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                        Don't have an account? <a href="/ecommerce-store/authentication/signup.cfm" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Sign Up here</a>
                    </p>
                </form>
    ...
    <script>
        $(document).ready(function(){
            $('#logInForm').on('submit', function(e){
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ecommerce-store/authentication/auth.cfc?method=login',
                    data: {
                        email: email,
                        password: password,
                    },
                    success: function(logInResponse) {
                        var parsed = $.parseJSON(logInResponse);
                            if(parsed.code == 200){
